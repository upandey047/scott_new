...
class Converter:
    def __init__(self, config):
        ...
        if args.get("viewport"):
            self.viewport = map(
                lambda s: float(s), args.get("viewport").split(" ")
            )
        else:
            self.viewport = False
        ...
        projString = (
            "+proj=" + str(self.projection) + " +a=6381372 +b=6381372 +lat_0=0"
        )
    ...
    def convert(self, data_source, output_file):
        codes = map(
            lambda g: g.properties[self.config["code_field"]],
                ...
                for ring in rings:
                    for pointIndex in range(len(ring.coords)):
                        ...
                        if pointIndex == 0:
                            path += "M" + str(
                                round(
                                    (point[0] - bbox[0]) / scale + left,
                                    self.precision,
                                )
                            )
                            path += "," + str(
                                round(
                                    (bbox[3] - point[1]) / scale + top,
                                    self.precision,
                                )
                            )
                        else:
...
class DataSource:
    ...
    def load_data(self):
        ...
        if "filter" in self.config and self.config["filter"] is not None:
            self.layer.SetAttributeFilter(
                self.config["filter"].encode("ascii")
            )
    ...
    def create_grammar(self):
        root_table = SymbolTable(
            "root",
            map(
                lambda f: Bind(f["name"], GeometryProperty(f["name"])),
                self.fields,
            ),
        )
...
class PolygonSimplifier:
    ...
    def simplifyRing(self, ring):
        ...
        if not isPivot:
            simpleRing = shapely.geometry.LineString(coords).simplify(
                self.tolerance
            )
...
class Processor:
    ...
    def union(self, config, data_source):
        ...
        for geometry in data_source.geometries:
            if geometry.properties[config["by"]] in groups:
                groups[geometry.properties[config["by"]]]["geoms"].append(
                    geometry.geom
                )
            else:
    ...
    def merge(self, config, data_source):
        ...
        for rule in config["rules"]:
            ...
            new_geometries.append(
                Geometry(
                    shapely.ops.cascaded_union(geometries), rule["fields"]
                )
            )
        ...
        for geometry in data_source.geometries:
            if geometry.properties[config["on"]] in data:
                geometry.properties.update(
                    data[geometry.properties[config["on"]]]
                )
    ...
    def remove_small_polygons(self, config, data_source):
        for geometry in data_source.geometries:
            if isinstance(
                geometry.geom, shapely.geometry.multipolygon.MultiPolygon
            ):
                polygons = geometry.geom.geoms
            else:
                polygons = [geometry.geom]
            polygons = filter(
                lambda p: p.area > config["minimal_area"], polygons
            )
            if len(polygons) > 0:
                geometry.geom = shapely.geometry.multipolygon.MultiPolygon(
                    polygons
